...
def tensor2numpy_tuple(x):
    return x[0].cpu().data.numpy() if x[0].is_cuda else x[0].data.numpy(), \
           x[1].cpu().data.numpy() if x[1].is_cuda else x[1].data.numpy(), \
           x[2].cpu().data.numpy() if x[2].is_cuda else x[2].data.numpy()
