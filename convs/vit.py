...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.fc1 = nn.Linear(dim, mlp_hidden_dim)  # Linear(in_features=768, out_features=3072, bias=True)
        self.fc2 = nn.Linear(mlp_hidden_dim, dim)  # Linear(in_features=3072, out_features=768, bias=True)
        self.act = act_layer()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, num_patches + self.num_tokens, embed_dim))  # torch.Size([1, 197, 768])
    ...
    def forward_train(self, x):
        B = x.shape[0]  # x: torch.Size([48, 3, 224, 224])
        x = self.patch_embed(x)  # torch.Size([48, 196, 768])

        cls_tokens = self.cls_token.expand(B, -1, -1)  # torch.Size([48, 1, 768])
        x = torch.cat((cls_tokens, x), dim=1)  # torch.Size([48, 197, 768])
        x = x + self.pos_embed  # self.pos_embed: torch.Size([1, 197, 768])
        x = self.pos_drop(x)  # torch.Size([48, 197, 768])

        if self.global_pool:
            x = x[:, 1:, :].mean(dim=1)
            outcome = self.fc_norm(x)
        else:
            x = self.norm(x)  # torch.Size([48, 197, 768])
            outcome = x[:, 0]  # torch.Size([48, 768])
    ...
    def forward(self, x, test=False, use_init_ptm=False):
        if not test:
            output = self.forward_train(x)  # torch.Size([48, 768])
        else:
...
def vit_base_patch16_224_in21k_ease_bkb(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
